...
int main(void) {
    ...
    assert(strcmp(buffer_a, "foo" LIBPATH_SEPARATOR "bar" LIBPATH_SEPARATOR "baz") == 0);
    assert(buffer_a[11] == '\0');

    assert(libpath_join_path(buffer_b, 12, "foo", "bar", "baz", NULL) == 11);
    assert(strcmp(buffer_b, "foo" LIBPATH_SEPARATOR "bar" LIBPATH_SEPARATOR "baz") == 0);
    assert(buffer_b[11] == '\0');

    /* Should not be able to be filled */
    assert(libpath_join_path(buffer_c, 6, "foo", "bar", "baz", NULL) == 6);
    assert(strcmp(buffer_c, "foo" LIBPATH_SEPARATOR "ba") == 0);
    assert(buffer_c[6] == '\0');
